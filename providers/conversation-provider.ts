...
export const [ConversationProvider, useConversation] = createContextHook(() => {
  ...
  const saveConversations = useCallback(async (newConversations: Conversation[]) => {
    try {
      await AsyncStorage.setItem('ifs_conversations', JSON.stringify(newConversations));
    } catch (error) {
  ...
  const loadConversations = useCallback(async () => {
    try {
      ...
      const storedConversations = await AsyncStorage.getItem('ifs_conversations');
      ...
      if (oldMessages) {
        ...
        if (parsedMessages.length > 0) {
          const migratedConversation: Conversation = {
            ...
            updatedAt: parsedMessages[parsedMessages.length - 1]?.timestamp || new Date(),
          };
  ...
  const saveSettings = useCallback(async (newSettings: ConversationSettings) => {
    try {
  ...
  const createProxyProvider = useCallback(() => createAIProvider({ provider: 'proxy' }), []);

  const createNewConversation = useCallback(async (systemPrompt?: string): Promise<string> => {
  ...
  const sendMessage = useCallback(async (content: string): Promise<string | null> => {
    ...
    try {
      const systemPrompt = targetConversation?.systemPrompt || IFS_SYSTEM_PROMPT;
      ...
      const result = await provider.generateText({ messages: conversationMessages });
      ...
      const finalConversations = updatedConversations.map(conv => 
        conv.id === targetConversationId 
          ? { 
              ...
              updatedAt: new Date(),
              title: conv.messages.length === 0 ? generateConversationTitle(finalMessages) : conv.title
            }
          : conv
      );
  ...
  }, [conversations, currentConversationId, createProxyProvider, createNewConversation, saveConversations, generateConversationTitle]);
  ...
  const sendMessageStream = useCallback(async (
    ...
    try {
      ...
      for await (const chunk of provider.generateTextStream({ messages: conversationMessages })) {
  ...
  const updateConversationSystemPrompt = useCallback(async (conversationId: string, systemPrompt: string) => {
    const updatedConversations = conversations.map(conv => 
      conv.id === conversationId 
